...
import org.big.pete.sft.domain.{Wallet, WalletEdit, Category, CategoryDeleteStrategies, EnhancedAccount, Account, AccountCurrency, AccountDeleteStrategy, ShiftStrategy, ShiftStrategyPerCurrency}
...
trait DataUpdate extends DataLoad {
  ...
  def saveAccount(oldPermalink: Option[String], id: Option[Int], name: String, permalink: String): Callback = {
    ...
    val payload = if (id.isDefined)
      BPJson.write(WalletEdit(oldPermalink.get, id.get, name, permalink, None))
    else
      BPJson.write(Wallet(-1, name, permalink, None))
